...
const login = (_privateKey, app) => {
  ...
  const storageKey = `${app.network}:cndl:privateKey`
  const privateKey = _privateKey || app.env.storage.getItem(storageKey)
  let account

  if (privateKey) {
    account = app.env.web3Cndl.eth.accounts.privateKeyToAccount(privateKey)
  }
  else {
    account = app.env.web3Cndl.eth.accounts.create()
  }

  app.env.web3Cndl.eth.accounts.wallet.add(account.privateKey)

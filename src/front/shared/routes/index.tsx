...
const routes = (
  <ScrollToTop>
    <Switch>
      ...
      <Route path={`/:ticker(btc|eth|bnb|matic|arbeth|xdai|ftm|ghost|next)/tx/:tx?`} component={Transaction} />
      <Route path={`/:token(token)/:ticker/tx/:tx?`} component={Transaction} />

      <Route
        path={`/:ticker(btc|eth|bnb|matic|arbeth|xdai|ftm|ghost|next)/:address/:action(receive|send)?`}

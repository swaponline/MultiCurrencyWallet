...
const routes = (
  <ScrollToTop>
    <Switch>
      ...
      <Route path={`/:ticker(btc|eth|bnb|cndl|matic|arbeth|aureth|xdai|ftm|avax|movr|one|phi|ghost|next)/tx/:tx?`} component={Transaction} />
      <Route path={`/:token(token)/:ticker/tx/:tx?`} component={Transaction} />

      <Route
        path={`/:ticker(btc|eth|bnb|cndl|matic|arbeth|aureth|xdai|ftm|avax|movr|one|phi|ghost|next)/:address/:action(receive|send)?`}

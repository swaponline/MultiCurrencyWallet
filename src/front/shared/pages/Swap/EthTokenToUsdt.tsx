      ...
      <div>
        ...
        {flow.isWaitingForOwner && (
          <Fragment>
            <h3>
              <FormattedMessage
                id="EthTokenToUsdt84"
                defaultMessage="Waiting for other user when he connect to the order"
              />
            </h3>
            <InlineLoader />
          </Fragment>
        ...
        {(flow.step === 1 || flow.isMeSigned) && (
          <h3>
            <FormattedMessage
              id="EthTokenToUsdt93"
              defaultMessage="1. Please confirm your participation to begin the deal"
            />
          </h3>
        )}
        {flow.step === 1 && (
          <Fragment>
            ...
            {(flow.isSignFetching || flow.signTransactionHash) && (
              <Fragment>
                <h4>
                  <FormattedMessage
                    id="EthTokenToUsdt118"
                    defaultMessage="Please wait. Confirmation processing"
                  />
                </h4>
                ...
                {flow.isSignFetching && <InlineLoader />}
              </Fragment>
            )}
          </Fragment>
        )}
        ...
        {flow.isMeSigned && (
          <Fragment>
            <h3>
              <FormattedMessage
                id="EthTokenToUsdt154"
                defaultMessage="2. Waiting for USDT Owner to create a Secret Key, create BTC Omni Script and charge it"
              />
            </h3>
            {flow.step === 2 && <InlineLoader />}

            {flow.secretHash && flow.usdtScriptValues && (
              <Fragment>
                <h3>
                  <FormattedMessage
                    id="EthTokenToUsdt166"
                    defaultMessage="3. Bitcoin Omni Script created and charged. Please check the information below"
                  />
                </h3>
                <div>
                ...
                <Fragment>
                  {flow.usdtScriptValues && (
                    <span onClick={this.toggleBitcoinScript}>
                      <FormattedMessage
                        id="EthTokenToUsdt192"
                        defaultMessage="Show bitcoin script "
                      />
                    </span>
                  )}
    ...
    bitcoin.core.opcodes.OP_ENDIF,
            ...
            {flow.step === 4 && !flow.isBalanceEnough && !flow.isBalanceFetching && (
              <Fragment>
                <h3>
                  <FormattedMessage
                    id="EthTokenToUsdt249"
                    defaultMessage="Not enough money for this swap. Please fund the balance"
                  />
                </h3>
                <div>
                  <div>
                  ...
                  <div>
                    ...
                    <a
                      href={`${config.link.usdt}/address/${currencyAddress}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      {currencyAddress}
            ...
            {(flow.step >= 5 || flow.isEthContractFunded) && (
              <h3>
                <FormattedMessage
                  id="EthTokenToUsdt285"
                  defaultMessage="4. Creating Ethereum Contract.{br}Please wait, it can take a few minutes"
                  values={{ br: <br /> }}
                />
              </h3>
            )}
            ...
            {(flow.step === 6 || flow.isEthWithdrawn) && (
              <Fragment>
                <h3>
                  <FormattedMessage
                    id="EthTokenToUsdt331"
                    defaultMessage="5. Waiting for BTC Owner to add a Secret Key to ETH Contact"
                  />
                </h3>
                {!flow.isEthWithdrawn && <InlineLoader />}
              </Fragment>
            )}
            ...
            {flow.usdtSwapWithdrawTransactionHash && (
              <div>
                <FormattedMessage
                  id="EthTokenToUsdt352"
                  defaultMessage="USDT withdrawal transaction: "
                />
                <strong>
                  <a
            ...
            {flow.step === 7 && <InlineLoader />}

            {flow.isBtcWithdrawn && (
              <Fragment>
                <h3>
                  <FormattedMessage
                    id="EthTokenToUsdt377"
                    defaultMessage="7. USDT was transferred to your wallet. Check the balance."
                  />
                </h3>
                <h2>
                  <FormattedMessage
                    id="EthTokenToUsdt380"

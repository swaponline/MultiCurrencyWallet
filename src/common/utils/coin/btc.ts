...
const estimateFeeRate = async (options) => {
  const { speed = 'fast', method } = options
  ...
  const link = 'https://wiki.swaponline.io/blockcyper.php'
  const defaultRate = {
    slow: 5 * 1e3,
    normal: 15 * 1e3,
    fast: 30 * 1e3,
  ...
  try {
    ...
    console.error("Can't fetch ${link}")
...
const calculateTxSize = async (params: CalculateTxSizeParams) => {
  ...
  if (method === 'swap') {
    ...
    let txSize = 400 // default tx size for swap method
    ...
    console.group('Common > coin >%c btc > calculateTxSize (swap)', 'color: green;')
  ...
  const defaultTxSize = 226 // default tx size for 1 txIn and 2 txOut
  if (fixed) {
...
const estimateFeeValue = async (params: EstimateFeeValueParams) => {
  const {
    feeRate: _feeRate,
    inSatoshis,
    speed,
  ...
  if(method === 'swap') {
    ...
    if (!_txSize && !address) {
      ...
      const txSize = _txSize || await calculateTxSize({ address, method, NETWORK })
  ...
  let unspents: IBtcUnspent[] = await fetchUnspents({
    address,
    NETWORK,
  })
  ...
  const feeRate = _feeRate || await estimateFeeRate({ speed })

...
//     }
//
//     this.tokenContract = new web3.eth.Contract(abi, this.contractAddress, options)
//     this.ERC20         = new web3.eth.Contract(abi, this.contractAddress)
//   }
//
//   login(privateKey) {
//     let data
...
//
//     this.setupContract(data.address, this.contractAddress, this.name, this.decimals)
//   }
//
//
...
//   getBalance() {
//     const { user: { ethData: { address } } } = getState()
//     const url = `${config.api.etherscan}?module=account&action=tokenbalance&contractaddress=${this.contractAddress}&address=${address}`
//
//     return request.get(url)
//       .then(({ result }) => {
//         const amount = new BigNumber(String(result)).dividedBy(new BigNumber(String(10)).pow(this.decimals)).toNumber()
//
//         reducers.user.setTokenBalance({ name: this.name, amount })
//         return amount
//       }).catch(r => console.error('Token service isn\'t available, try later'))
//   }
//
//   fetchBalance = (address) =>
//     request.get(`https://rinkeby.etherscan.io/api?module=account&action=tokenbalance&contractaddress=${this.tokenAddress}&address=${address}`)
//       .then(({ result }) => {
//         const amount = new BigNumber(String(result)).dividedBy(new BigNumber(String(10)).pow(this.decimals)).toNumber()
//
//         return amount
...
//
//   send = (to, amount) => {
//     const newAmount = new BigNumber(String(amount)).times(new BigNumber(10).pow(this.decimals)).integerValue()
//
//     return new Promise(async (resolve, reject) => {
//       const receipt = await this.tokenContract.methods.transfer(to, newAmount).send()
...
//     return new Promise(async (resolve, reject) => {
//       try {
//         const result = await this.ERC20.methods.approve(config.token.contract, newAmount).send({
//           from: address,
//           gas: `${config.services.web3.gas}`,
//           gasPrice: `${config.services.web3.gasPrice}`,
//         })

...
export default class BtcToEthToken extends Component {
  ...
  constructor({ swap }) {
    ...
    this.state = {
      ...
      destinationBuyAddress: swapApp.services.auth.accounts.eth.address
    }
  }
  ...
  confirmAddress = () => {
    ...
    this.setState( {
      destinationAddressConfirmed: true
    } )
  }
  ...
  render() {
    ...
    const { secret, flow, enabledButton, destinationAddressConfirmed } = this.state
    ...
    return (
      <div>
        ...
        {
          (!flow.isWaitingForOwner && !destinationAddressConfirmed && (flow.step === 1 || flow.step === 2 )) && (
          <Fragment>
            <FormattedMessage id="BtcToEthTokenAddress1" defaultMessage="Confirm destination address for tokens">
              {message => <h3>{message}</h3>}
            </FormattedMessage>
            <Input valueLink={linked.destinationBuyAddress} styleName="input" pattern="0-9a-zA-Z" />
            <hr />
        ...
        {
          (destinationAddressConfirmed || flow.step >= 3) && (
            <Fragment>
            {

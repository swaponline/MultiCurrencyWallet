...
import styles from './SwapWindow.scss'
...
export default class SwapWindow extends Component {

  static defaultProps = {
    orders: [],
  }

  static getDerivedStateFromProps({ allOrders, orders, history }, { haveCurrency, getCurrency }) {
  ...
  constructor({ tokensData, allCurrencyies, currenciesData, intl: { locale }, history, ...props }) {
    super()
  ...
  render() {
    ...
    const isWidgetLink = window.location.pathname.includes('/exchange') && window.location.hash === '#widget' // заменить виндоу на пропс

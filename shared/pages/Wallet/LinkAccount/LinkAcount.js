...
const LinkAccount = ({ type, children, address }) => (
  <Fragment>
    ...
    { type.toLowerCase() === 'swap' && <Href tab={`${config.link.etherscan}/token/${config.tokens.swap}?a=${address}`} >{children}</Href> }
    { type.toLowerCase() === 'noxon' && <Href tab={`${config.link.etherscan}/token/${config.tokens.noxon}?a=${address}`} >{children}</Href> }

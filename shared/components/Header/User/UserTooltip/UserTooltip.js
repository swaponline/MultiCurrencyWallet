        ...
        <div styleName="column">
          {feeds.length < 3 ? (
            feeds.map(row => {
              const {
                request,
                content: { buyAmount, buyCurrency, sellAmount, sellCurrency },
                id,
                peer: ownerPeer
              } = row;
              const currencyBalance = this.state.allCurrencyies.find(
                item => item.currency === sellCurrency
              ).balance;
              const sellAmountPlusFee = BigNumber(
                this.state.estimatedFeeValues[sellCurrency.toLowerCase()]
              ).plus(sellAmount);
                  ...
                  <div styleName="userTooltip">
                    <div key={peer}>
                      ...
                      <div styleName="currency">
                        <span>
                          {buyAmount.toFixed(5)}{" "}
                          <span styleName="coin">{buyCurrency}</span>
                        </span>
                        ...
                        <span>
                          ...
                          <span styleName="coin">{sellCurrency}</span>
                        </span>
                      </div>
                    </div>
                    <span
                      styleName="decline"
                      onClick={() => this.props.declineRequest(id, peer)}
                    />
                    <div
                      styleName="checked"
                      onClick={() =>
                        this.props.acceptRequest(
                          id,
                          peer,
                          `${links.swap}/${sellCurrency}-${buyCurrency}/${id}`
                        )
                      }
                    />
                    <TimerButton
            ...
            <div styleName="feed">
              <Link to={links.feed}>
                <FormattedMessage
                  id="userTooltip71"
                  defaultMessage="Go to the feed page"
                />
              </Link>
            </div>
          )}

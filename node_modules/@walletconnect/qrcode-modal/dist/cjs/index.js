...
function getMobileRegistryEntry(name) {
  return MOBILE_REGISTRY.filter(function (entry) { return entry.name.toLowerCase().includes(name.toLowerCase()); })[0];
}
...
function MobileLinkDisplay(props) {
  ...
  var pageLinks = links.filter(function (_, index) { return index + 1 >= range[0] && index + 1 <= range[1]; });
  ...
  }, ios ? pageLinks.map(function (entry) {

...
var PublicKeyHash256Input = Input.PublicKeyHash256;
var PublicKeyHashInput = Input.PublicKeyHash;
var PublicKeyInput = Input.PublicKey;
var MultiSigScriptHashInput = Input.MultiSigScriptHash;
var MultiSigScriptHash256Input = Input.MultiSigScriptHash256;
...
Transaction.prototype._fromNonP2SH = function(utxo) {
  ...
  } else if (utxo.script.isPublicKeyHash256Out()) {
    clazz = PublicKeyHash256Input;
  } else {
    clazz = Input;
  }
...
Transaction.prototype._fromMultisigUtxo = function(utxo, pubkeys, threshold, nestedWitness, opts) {
  ...
  } else if (utxo.script.isScriptHashOut256()) {
    clazz = MultiSigScriptHash256Input;
  } else {
 ...
 * @param {string} cold staking address.
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.change = function(address, coldStakingAddress) {
  $.checkArgument(address, 'address is required');  
  this._changeScript = Script.fromAddress(address, coldStakingAddress);
  this._updateChangeOutput();
  return this;
};
...
Transaction.prototype.getSignatures = function(privKey, sigtype) {
  ...
  var results = [];
  var hashData = {};
  var publicKeyBuffer = privKey.publicKey.toBuffer();
  _.each(this.inputs, function forEachInput(input, index) {
    var hashType = input instanceof PublicKeyHash256Input ? 'sha256' : 'sha256ripemd160';
    if(!hashData[hashType]){
      hashData[hashType] = Hash[hashType](publicKeyBuffer);
    }
    _.each(input.getSignatures(transaction, privKey, index, sigtype, hashData[hashType]), function(signature) {

...
Input.prototype._fromObject = function(params) {
  ...
  if (!_.isUndefined(params.witnesses))
...
Input.fromBufferReader = function(br) {
  ...
  if (input.outputIndex == ANON_MARKER) {
    input.data = [];
    var sizeStack = br.readVarintNum();

    for (var k = 0; k < sizeStack; k++) {
      var sizeData = br.readVarintNum();
      var stackData;
      if (sizeData !== 0)
        stackData = br.read(sizeData);
      else
        stackData = new buffer.Buffer([]);
      input.data.push(stackData);
    }
  }
...
Input.prototype.toBufferWriter = function(writer) {
  ...
  if (this.outputIndex == ANON_MARKER) {
    var sizeStack = this.data.length;
    writer.writeVarintNum(sizeStack);

    for (var k = 0; k < sizeStack; k++) {
      var sizeData = this.data[k].length;
      writer.writeVarintNum(sizeData);
      writer.write(this.data[k]);
    }
  }

...
**Note:** Tech-spec specifies `verify: bash` for this task, indicating verification through build/test commands rather than traditional TDD. The tests below ensure behavior is correct, but may be written alongside implementation or after, unlike pure TDD tasks where tests come first.

Tests to write (alongside or before implementation):
...
**Code files:**
- `src/components/Web3ReactManager/index.tsx` — Component to modify (**WARNING: this is in unifactory external repo, not MCW**)
- `src/hooks/index.ts` — Contains useEagerConnect hook that will be called from event handler (**WARNING: this is in unifactory external repo, not MCW**)
...
- `src/components/Web3ReactManager/index.tsx` (**WARNING: unifactory external repo**):

...
This is a single-line configuration change without logic changes. Unit tests for the bridge provider properties will be added in Task 7 (MCW bridge unit tests), which will verify the `isMetaMask` flag is set correctly along with other provider properties.

## Acceptance Criteria

- [ ] AC1: File `src/front/client/wallet-apps-bridge-client.js` on branch `issue-5268-apps-layout` has `isMetaMask: true` on line 111

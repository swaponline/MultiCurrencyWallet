...
No unit tests needed for this task. This is HTML/JavaScript injection that will be verified via:
- E2E test (Task 8) checking bridge client loaded in iframe
- Manual verification (user inspection of view-source)

The inline script is simple, pure vanilla JavaScript with no testable business logic beyond what E2E already covers.
...
- `public/index.html` — unifactory repo (current structure needs to be inspected when agent has access to unifactory repo)
...
- Use `async = false` on dynamically created script to ensure it loads before React bundle
